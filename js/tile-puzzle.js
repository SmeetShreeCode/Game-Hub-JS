...
let imagePath = "images/Puzzle-craft/L1.webp";   // default
let img = new Image();
img.src = imagePath;
...
function renderTiles() {
    ...
    img.onload = () => {
        tileArray.forEach((value, index) => {
            ...
            if (value !== 0) {
                // compute correct image crop
                let correctRow = Math.floor((value - 1) / CUBE);
                let correctCol = (value - 1) % CUBE;

                tile.style.backgroundImage = `url('${imagePath}')`;
                tile.style.backgroundSize = `${CUBE * 100}% ${CUBE * 100}%`;
                tile.style.backgroundPosition =
                    `${-(correctCol * tileSize)}px ${-(correctRow * tileSize)}px`;
            }
...
function loadLevel() {
    imagePath = document.getElementById("level-select").value;
    img.src = imagePath;  // reload image
    shuffleTiles();       // restart game
}
