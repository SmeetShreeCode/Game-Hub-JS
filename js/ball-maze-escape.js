...
class GameScene extends Phaser.Scene {
    ...
    createRing(cx, cy, ringConfig) {
        ...
        const gapCenterDeg = ringConfig.GAP_OFFSET ?? 270;
        const gapCenterRad = Phaser.Math.DegToRad(gapCenterDeg);
        const halfGap = Phaser.Math.DegToRad(GAP_ANGLE / 2);

        // Physics arc: start and end angles, skipping the gap
        const startAngle = gapCenterRad + halfGap;
        const endAngle = gapCenterRad + Math.PI * 2 - halfGap;
