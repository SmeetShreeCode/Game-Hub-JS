module.exports = function (orbital) {
...
// Socket.IO connection
    orbital.on('connection', (socket) => {
        console.log('a user connected to the orbitAlliance');

        orbital.emit('updatePlayers', backEndPlayers);
        ...
        socket.on('disconnect', (reason) => {
            console.log('user disconnected to the orbitAlliance');
            console.log(reason);
...
//backend ticker
    setInterval(() => {
        ...
        orbital.emit('updateProjectiles', backEndProjectiles);
        orbital.emit('updatePlayers', backEndPlayers);
    }, 15);
};
